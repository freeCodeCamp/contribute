...
- For blocks, change the `isUpcomingChange` property to `false` in the block's structure file (`curriculum/structure/blocks/[block-name].json`).
...
- Lowercase annotations: use lowercase for annotations, even if the base text is a proper noun or is the first word of a sentence
