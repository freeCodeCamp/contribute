...
1.  Package a new version of the extension:
    ...
    ```bash
    npm run pack -- --no-git-tag-version <tag_type>
    ```

    Where `<tag_type>` is one of: `major`, `minor`, `patch`.

    The `--no-git-tag-version` flag updates the package version without creating a Git commit or tag. These are created in the following steps.
...
2. Navigate to the
   ...
   page](https://marketplace.visualstudio.com/manage/publishers/freecodecamp)
