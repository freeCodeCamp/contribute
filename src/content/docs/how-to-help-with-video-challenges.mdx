...
## Creating a Video Challenge

To add a video challenge to an existing block, change to the block's directory and run:

```bash
cd curriculum/challenges/english/blocks/<block>
pnpm run create-next-challenge
```

Select `video` when prompted for the challenge type, then enter the challenge's dashed name and title.

The script generates an id, creates the challenge file with `challengeType: 11`, and appends it to `challengeOrder` in `curriculum/structure/blocks/<block>.json`.

In the generated file, replace the `videoId` placeholder with the YouTube video id and fill in the description, question, answers, and solution number.
