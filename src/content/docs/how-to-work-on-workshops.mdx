...
**DOM vs non-DOM steps:**

For HTML/CSS workshops (`challengeType: 0`), `--before-all--` is injected as a `<script>` tag into the sandboxed iframe when the page loads. It runs with full access to DOM APIs and global test helpers like `__FakeTimers` and `$` (jQuery).

For JavaScript and Python workshops (`challengeType: 1` and `20`), all hooks run in a Web Worker context. There is no DOM access.
