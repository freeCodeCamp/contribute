...
For HTML/CSS labs (`challengeType: 25`), `--before-all--` is injected as a `<script>` tag into the sandboxed iframe when the page loads. It runs with full access to DOM APIs and global test helpers like `__FakeTimers` and `$` (jQuery).

For JavaScript and Python labs (`challengeType: 26` and `27`), all hooks run in a Web Worker context. There is no DOM access.
