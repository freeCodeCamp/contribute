...
Labs that have a visual output (i.e. HTML-based) can have a demo project, in which case the frontmatter includes `demoType: onClick`. It is not mandatory even for HTML labs.
...
For JavaScript and Python labs (`challengeType: 26` and `27`), `--before-all--` runs once when the Worker initialises, before any test. User code is then re-evaluated as part of each individual test (concatenated with `--before-each--` and the test in a single `eval`):
...
There is no DOM access in Worker-based labs.
