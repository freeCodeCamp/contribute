...
```js
// This code will run before the user code and tests. For example:
window.fetch = () =>
  Promise.resolve({ json: () => Promise.resolve({ data: 'example' }) });
// Warning: this is currently only supported in challenges that render HTML.
...
```js
// This code will run before each test execution. For example:
globalThis.testVariable = 'initial value';
// Note: the --before-each-- section is optional.
// Warning: __helpers are not available in --before-each-- sections.
```
...
## --before-user-code-- (deprecated)

```js
Code evaluated before the user's code.
```

## --after-user-code-- (deprecated)
...
2. You can also test a block or a superblock of challenges with these commands
   ...
   Use the block's `dashedName` (found in the block's JSON structure file) for the `FCC_BLOCK` value.
