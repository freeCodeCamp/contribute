...
```js
// Note: the --before-all-- section is optional.
// Warning: __helpers are not available in --before-all-- sections.
// This code will run before the learner's code and tests. For example, initializing fake timers
let clock = __FakeTimers.install();
// It is necessary for HTML challenges when something needs to be mocked before the learner's code is evaluated. For example, fake timers need to be setup ahead of time if the learner's code will call setTimeout.
```

# --before-each--

```js
// Note: the --before-each-- section is optional.
// Precisely when this is evaluated depends on the type of challenge. HTML challenges run it after the learner's code, but before each test. In all other challenges the order is before-each, learner code and then test code.
...
```js
// Note: the --after-each-- section is optional.
...
```js
// Note: the --after-all-- section is optional.
