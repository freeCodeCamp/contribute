...
:::note
Theory lesson content is authored by staff only. If you would like to propose a new lesson, please open an issue so the curriculum team can review and triage the request.
:::
...
For longer lessons, use headings to divide the content into logical sections. The highest heading level in the lesson text is `##`. Keep headings short and use Title Case, such as `## Sorting Strings` or `### Case-Insensitive Sorting`. Place them before major subtopics to make the lesson easier to scan.
