...
- Activity is defined as: Comments requesting an update on the PR and triages like `status: waiting update` label, etc.
...
1. **Challenge instruction edits**
   ...
   If the changes look good, please ensure to leave an approval with a "LGTM" (Looks Good To Me) comment. Once the pull request meets the [merge requirements](#merging-a-valid-pull-request), you can go ahead and merge it.
...
2. **Challenge code edits**
   ...
   If the changes look good, please ensure to leave an approval with an "LGTM" comment. Once the pull request meets the [merge requirements](#merging-a-valid-pull-request), you can go ahead and merge it.
...
The `freeCodeCamp/freeCodeCamp` repository requires at least one approving review, including approval from the required code owners. All required CI checks must pass, and all review conversations must be resolved before merging. For bigger changes, we recommend getting multiple approvals from moderators or the dev-team.

Once the pull request meets these requirements and GitHub shows no remaining merge blockers, you can go ahead and merge it. Be sure to use the default **"Squash and Merge"** option. This will squash all the pull requests commits down into a single commit, making the Git history much easier to read.
...
<details>
  <summary>First-time contributor badge on pull requests (screenshot)</summary>
  <br />
  <img
    src='https://i.imgur.com/dTQMjGM.png'
    alt='First time contributor badge on pull requests'
  />
</details>
...
If you need a second opinion on a pull request, go ahead and leave your comments on the pull request, then add the "status: discussing" label to the pull request.
...
[Direct link with filter applied](https://github.com/freeCodeCamp/freeCodeCamp/pulls?q=is%3Aopen+is%3Apr+-label%3A%22status%3A+blocked%22+-label%3A%22MERGE+CONFLICT%21%22+status%3Asuccess+draft%3Afalse)
...
### Using CamperChan for GitHub Tasks

CamperChan provides Discord commands for managing issues and pull requests in the freeCodeCamp GitHub organization. Each command takes a `repository` name, such as `freeCodeCamp`, and the issue or pull request `number`.

| Command               | Action                                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/github comment`     | Posts the predefined reply selected in `message`.                                                                                                                      |
| `/github close`       | Posts a closing comment and closes the issue or pull request. The optional `comment` selects a predefined reply; otherwise, the bot uses its standard closing message. |
| `/github add-labels`  | Adds the comma-separated `labels` while preserving existing labels.                                                                                                    |
| `/github sync-labels` | Replaces the entire label set with the comma-separated `labels`. Any existing labels omitted from the list are removed.                                                |

For issues, `/github close` accepts a `reason` of `completed`, `not planned`, or `duplicate`. It defaults to `not planned`. When choosing `duplicate`, supply `duplicate-of` with the number of the original issue in the same repository. These two options do not apply to pull requests. For pull requests, the optional `spam` flag adds the `spam` label.
...
If a user replies in a help thread for the freeCodeCamp curriculum with a solution, remove it and remind the user to provide hints and tips instead, using a suitable template or your own words.

If the OP (Original Poster) replies within a freeCodeCamp curriculum help thread with their final solution, blur it and explain why, using a suitable template or your own words.

If a user creates a thread asking for feedback on a solution, move the thread to the feedback subforum and blur the solution, as necessary. If the user is only posting the solution to show it off, then unlist the thread and explain why, using a suitable template or your own words.
...
- When trying to insert a template in a response, if the menu doesn't open (after clicking on the gear), click on the text area first then try it again.
...
:::note
CamperChan serves as our moderation bot. Type `/` in a server channel to see its slash commands. It also provides message commands, including `report` and `snippet`, through Discord's message context menu.
:::
...
2. **If the camper clearly and intentionally violates the [Code of Conduct](https://code-of-conduct.freecodecamp.org), the moderator will proceed as follows:**
   ...
   For minor offences, a warning may be issued with the `/moderation warn` command. For more egregious offences, you can remove the member from the server temporarily with the `/moderation kick` command, or permanently with the `/moderation ban` command. In some cases, a member may just need some time to cool off and collect their thoughts - the `/moderation mute` command allows you to prevent them from engaging with our community for a set period of time. A muted member can see the conversation, but cannot post messages or add reactions.

   `/moderation ban` also deletes the member's messages from the previous day. `/moderation mute` requires a `duration` and a `unit` of minutes, hours, days, or weeks, with a maximum duration of 28 days.

   Moderation commands that take action against a user require a `reason` parameter, which should be a short explanation of why the action was taken. Moderation actions done with the bot will be logged in `#mod-actions`, which allows us all to stay on the same page. As such, we should avoid using Discord's built-in moderation tools, as they will not be logged.

   :::caution
   The reason provided to a moderation command will also be included in the DM notification to the camper. Please remember to be professional here. DM delivery can fail without preventing the moderation action; check the notification status in the action log.
   :::
...
3. **Creating a private discussion**
   ...
   - Call the `/management private` command, where `target` is the camper you want to open a private channel with.
   ...
   - When the conversation is complete, click the `❌ Close` button _on the first message in the private channel_. The bot sends a text transcript to `#mod-actions` before deleting the channel.

4. **Deleting messages**

   Our moderation bot is configured to log deleted messages automatically in the `#message-logs` channel. CamperChan also logs message edits there, including a diff of the text, and records newly created threads. If a message is not in line with our Code of Conduct, or otherwise not appropriate for our community, you are generally safe to delete it.

   :::note
   If the message contains content that violates Discord's terms of service, right-click the message on desktop or press and hold it on mobile, then select **Report Message** to report it to Discord **prior to** deleting it.
   :::

5. **Don't threaten to take action**

   If a camper breaks the [Code of Conduct](https://code-of-conduct.freecodecamp.org), don't threaten to take moderator action, and never warn them in public. Instead, talk to them privately using the bot's `/management private` command, or use the bot's moderation commands.
...
:::note
Users who have been banned or muted can appeal their moderation actions. As a moderator, you can use `/moderation unban` and `/moderation unmute` commands when processing approved appeals. You can also use `/moderation history` to see counts of the user's bans, kicks, mutes, warnings, unmutes, and unbans. For the reasons and dates of individual actions, consult `#mod-actions`. Ban notifications include an appeal form link.
:::

### Reports to the Moderation Team

Members can report a message to the freeCodeCamp moderation team using CamperChan's `report` message command. It opens a form asking why they are reporting the message. The bot sends the message details, reporter, and submitted reason to the configured report channel.

The report includes a **Message Link** button and an **Acknowledge** button. Selecting **Acknowledge** records your name on the report and removes the buttons. It does not take moderation action against the reported member.

For voice-channel incidents, members can use `/report` with `user` and `reason`. The reporter and the reported member must both be in the same voice channel.

These reports go to our moderation team. Discord's **Report Message** option sends a report to Discord itself.

### Moderation Tickets

Members can select **Open a Ticket** on the ticket post to open a private thread with the moderation team. Tickets are for moderation concerns, Code of Conduct questions, and questions or appeals about moderation actions. They are not for coding help, curriculum bug reports, or general questions.

The bot prevents a member from opening another ticket while they already have an active ticket in that channel. When the discussion is complete, select **Close Ticket**. The bot removes the member, locks the thread, and archives it. The thread remains available to staff as a record of the conversation.

Tickets and `/management private` channels have different closing behavior: tickets are preserved as threads, while private channels are deleted after the bot sends their transcript to `#mod-actions`.

### Automatic Moderation

CamperChan automatically bans members it has permission to ban when it detects any of these triggers:

- A known phishing link.
- An unapproved Discord invite combined with an `@everyone` or `@here` mention.
- A message in the honeypot channel, where regular members should not post.

The bot attempts to notify the member, deletes their messages from the previous day, updates their moderation history, and records the trigger and message evidence in `#mod-actions`.

CamperChan also deletes messages containing attachments identified as neither images nor videos and posts an explanation in the channel. Members with **Manage Messages** permission and channels under the configured exempt category are excluded from this attachment restriction.

### Predefined Discord Replies

Use CamperChan's `snippet` message command to choose a predefined response from a dropdown. The bot posts the selected response as a reply to the original message. Available snippets cover situations such as code formatting, large code blocks, duplicate questions, and self-promotion.
...
If this happens on our chat server: Create a private channel for the user and the moderator team. This can be done with the bot's `/management private` command.
...
<div align='center'>
  <img src='/images/github/xkcd-free-speech.png' width='400' height='400' />
</div>
