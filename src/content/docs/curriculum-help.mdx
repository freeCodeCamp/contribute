...
This section outlines best practices for contributing to the freeCodeCamp curriculum. Following these guidelines helps ensure clarity, consistency, maintainability, and a great learning experience for campers.

### 1. Pre-indent Editable Regions

When creating or updating challenges that require learners to write code, ensure that editable regions are pre-indented correctly. This helps:

- Maintain consistent formatting.
- Prevent unnecessary syntax errors.
- Improve readability and learner experience.

### 2. Follow Consistent Formatting Standards

- Use consistent indentation (2 spaces for JavaScript, HTML, and CSS unless otherwise specified).
...
Good challenge instructions should:

- Clearly describe the goal of the challenge.
- Be concise and beginner-friendly.
- Avoid ambiguity.
- Provide enough context without revealing the solution.

### 4. Provide Helpful and Progressive Hints

Hints should:

- Guide learners toward the solution without giving it away.
- Progress from general guidance to more specific help.
- Encourage problem-solving and learning rather than copy-pasting.

### 5. Write Maintainable and Reliable Tests

When writing tests:

- Test behavior, not specific implementations.
- Avoid overly strict pattern matching where possible.
- Ensure error messages are clear and helpful.
- Keep tests resilient to small formatting differences.

Following these best practices ensures that the curriculum remains high quality, consistent, and easy to maintain for contributors and educators alike.
...
```js
await __helpers.retryingTest(callback, numberOfTries);
```
...
An example of its use can be found [in this file](https://github.com/freeCodeCamp/freeCodeCamp/blob/1ffd4568c4bda861d3a65ffaf1c39cb435b82648/curriculum/challenges/english/blocks/learn-fetch-and-promises-by-building-an-fcc-authors-page/641da803d9892447d059804e.md);
...
These helpers provide powerful Abstract Syntax Tree (AST) analysis capabilities for Python challenges.
...
The Python helpers also include some standalone utility functions for extracting specific information from Python code.
...
Available via `__helpers.python.getBlock(code, blockPattern)`, this function extracts code blocks matching specific patterns (if, for, while, etc.):
...
// Extract if block
const ifBlock = __helpers.python.getBlock(pythonCode, 'if');
// Returns the if statement block

// Extract for block
const forBlock = __helpers.python.getBlock(pythonCode, 'for');
// Returns the for loop block
```
