...
```js
const explorer = new Explorer('function foo() { return 42; }');
const { foo } = explorer.functions;
...
```js
const explorer = new Explorer('const a = maybeValue!;');
const { a } = explorer.variables;
...
```js
const explorer = new Explorer('function id<T>(x: T): T { return x; }');
const { id } = explorer.functions;
id.typeParameters[0].matches('T'); // true

const explorer2 = new Explorer('const m: Map<string, number> = new Map();');
...
```js
const explorer = new Explorer('class Foo { private readonly x: number; }');
const { Foo } = explorer.classes;
