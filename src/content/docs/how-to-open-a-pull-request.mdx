...
These are some common scopes:

- **UI**: Threads for addressing UX changes and improvements to the user interface.
- **curriculum**: Lessons, Challenges, Projects, and other Curricular Content in the curriculum directory.
- **a11y**: Threads for addressing accessibility issues.
- **i18n**: Language translation/internationalization. Often combined with a language type label.
- **tools/scripts**: Scripts for supporting development work, generating configuration and build artifacts, etc.
- **english for developers**: English Curriculum
- **archived coursework**: The scope includes archived coursework. This has a lower priority than the current curriculum.
