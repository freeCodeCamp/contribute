...
For challenges that do not have an HTML file, `--before-all--` runs once before any test. User code is then re-evaluated as part of each individual test (concatenated with `--before-each--` and the test in a single `eval`):
