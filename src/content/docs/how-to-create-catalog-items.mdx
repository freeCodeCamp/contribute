...
10. Add the superblock's entry to `intro.json`, following the [documented format](/curriculum-file-structure/#the-introjson-file). Include a `summary` for the catalog and entries for every block, even when reusing existing blocks. If the superblock uses chapters and modules, include their titles and any coming-soon module introductions.
...
You also need [FFmpeg](https://ffmpeg.org/download.html) installed to merge the separate video and audio streams selected by the command below. Depending on how you installed yt-dlp, you may need additional setup for YouTube support. Follow the [yt-dlp JavaScript setup guide](https://github.com/yt-dlp/yt-dlp/wiki/EJS) to check whether you need to install a JavaScript runtime or the challenge solver scripts.
