...
10. In `intro.json`, add the superblock's `title`, `summary` and `intro`. Under its `blocks` key, add a `title` and `intro` for every block. Copy these entries under the new superblock key even when reusing existing blocks.
