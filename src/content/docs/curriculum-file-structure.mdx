...
Each superblock has its own JSON file that defines which blocks belong to it:
...
- **`isPrivate`**: Boolean flag that controls build visibility
- **`tests`**: Array of required projects with their IDs and titles

### Build Control with `isPrivate`

The `isPrivate: true` property serves as a build control mechanism:

- **`isPrivate: true`**: The certification is excluded from the public build and will not appear on the live site
- **`isPrivate: false`**: The certification is included in all builds and appears on the live site

:::important
The `isPrivate` property is required for certification files due to schema validation. Certification files must have either `isPrivate` or `helpCategory`, but not both (XOR validation).
:::
...
5. Optionally, update the `certSlug` for the superblock in the same file.

   :::note
   Renaming a `certSlug` will change the URL for certifications and should only be done with careful consideration.
   :::

6. Update the `title` in `client/src/resources/cert-and-project-map.ts` to the new value.
