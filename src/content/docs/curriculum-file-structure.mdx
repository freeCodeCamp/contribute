...
│ │ ├─ <block>.json
```
## Seed Code Formatting Guidelines

When writing curriculum seed code, follow consistent formatting standards to ensure clarity, maintainability, and a smooth learning experience:
...
6. **If you renamed the `certSlug`**, [add redirects to the `serve.json` file](#setting-up-redirects) to redirect from old certification URLs to new ones:
   ```json
   {
...
10. If you renamed the `certSlug` in step 5, update the key of `certSlugTypeMap` in the same file.
11. Update the certificate name in the `legacyCerts` array of the `client/src/client-only-routes/show-project-links.tsx` if needed.
...
4. [**Add redirects** to the `serve.json` file](#setting-up-redirects) to redirect from old superblock URLs to new ones:
   ```json
   {
...
4. [**Add redirects** to the `serve.json` file](#setting-up-redirects) to redirect from old challenge URLs to new ones:
   ```json
   {
