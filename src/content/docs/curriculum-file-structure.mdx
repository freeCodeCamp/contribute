...
sidebar:
  label: The curriculum file structure
---
...
- `superBlock` : A superblock is a top level collection of challenges. Some superblocks correspond to certifications (e.g. Responsive Web Design), while others contain courses or additional practice.
...
Superblocks are classified in [`packages/shared/src/config/curriculum.ts`](https://github.com/freeCodeCamp/freeCodeCamp/blob/main/packages/shared/src/config/curriculum.ts):

- `SuperBlocks` maps superblock enum members to their URL slugs.
- `SuperBlockStage` defines the categories: `Core`, `English`, `Spanish`, `Chinese`, `Professional`, `Extra`, `Legacy`, `Upcoming`, `Next`, and `Catalog`.
- `superBlockStages` lists the superblocks assigned to each category.

These categories map to the groupings on the [Learn page](https://www.freecodecamp.org/learn). `Legacy` superblocks appear on the [Archive page](https://www.freecodecamp.org/learn/archive), and `Catalog` superblocks appear in the [Catalog](https://www.freecodecamp.org/catalog). `Upcoming` superblocks are hidden unless upcoming changes are enabled with `SHOW_UPCOMING_CHANGES=true`.

To find a superblock's category, look up its slug in `SuperBlocks`, then find the corresponding enum member in `superBlockStages`. For example, `javascript-v9` maps to `SuperBlocks.JsV9`, which appears in `superBlockStages[SuperBlockStage.Core]`.

Use `superBlockStages` to find category membership, the files in `curriculum/structure/superblocks/` to find chapters, modules, and blocks, and [`intro.json`](#the-introjson-file) to find display titles and introductory text.

### Catalog Topics and Difficulty

Catalog metadata is defined in [`packages/shared/src/config/catalog.ts`](https://github.com/freeCodeCamp/freeCodeCamp/blob/main/packages/shared/src/config/catalog.ts). The `Topic` enum defines the available topics, and the `Levels` enum defines the difficulty levels: `Beginner`, `Intermediate`, and `Advanced`.

The `catalog` array assigns each listed superblock a `topic`, a difficulty `level`, and a duration in `hours`. To find a course's metadata, look for its `SuperBlocks` enum member in an entry's `superBlock` field. For example, the entry for `SuperBlocks.BasicHtml` has `topic: Topic.Html` and `level: Levels.Beginner`.
...
When renaming curriculum items that change URLs, you **must** set up redirects to ensure existing links continue to work. This requires updating the file `serve.json` in the [client-config](https://github.com/freeCodeCamp/client-config) repo.
...
7. **If you renamed the certification slug**, [add redirects to the `serve.json` file](#setting-up-redirects) to redirect from old certification URLs to new ones:
   ```json
   {
...
6. [**Add redirects** to the `serve.json` file](#setting-up-redirects) to redirect from old superblock URLs to new ones:
   ```json
   {
...
3. [**Add redirects** to the `serve.json` file](#setting-up-redirects) to redirect from old challenge URLs to new ones:
   ```json
   {
