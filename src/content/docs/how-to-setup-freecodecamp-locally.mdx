...
#### Running node tests in Gitpod

Once you have a Gitpod instance running, open a terminal and then install the following
packages:
...
Then make sure Puppeteer has the correct Chrome version:

```bash
pnpm -F=curriculum install-puppeteer
```

Now the curriculum tests should run successfully. To see which test commands are available, go to [quick command reference](/how-to-contribute-to-the-codebase/#quick-commands-reference).

This completes the Gitpod setup. You can now skip to [Set up Syncing from Parent](#set-up-syncing-from-parent). Most parts of this guide applies to Gitpod workspaces, but be mindful of [how the URLs & Ports work within a Gitpod](https://www.gitpod.io/docs/configure/workspaces/ports) workspace.
...
<details>
  <summary>How to fork freeCodeCamp on GitHub (screenshot)</summary>
  <br />
  <img
    src='/images/github/how-to-fork-freeCodeCamp.gif'
    alt='How to fork freeCodeCamp on GitHub'
  />
</details>
...
3. (Optional) The localized versions of our curriculum are stored in the [i18n-curriculum repo.](https://github.com/freeCodeCamp/i18n-curriculum) If you intend to run a localized version of the curriculum, you need to download the files from there. If not, you can skip this step. Run:
