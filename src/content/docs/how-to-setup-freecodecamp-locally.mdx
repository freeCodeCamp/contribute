...
If you're setting up the project in WSL on Windows, run `pnpm run playwright:install-build-tools` before `pnpm install`. This command installs the system packages required by Playwright and Puppeteer, and may prompt for your `sudo` password.
