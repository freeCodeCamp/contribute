...
If you're setting up the project in WSL on Windows, run `pnpm run playwright:install-build-tools` before `pnpm install`. This may prompt for your `sudo` password while installing the system packages needed by Playwright and Puppeteer.
