...
- Open the 1st PR to configure GitHub Actions. You need to update 2 files:
  - `crowdin-download.client-ui.yml` (in the main repository)
  - `download-curriculum.yml` (in the i18n-curriculum repository)
...
You will need to add a step to the [`crowdin-download.client-ui.yml`](https://github.com/freeCodeCamp/freeCodeCamp/blob/main/.github/workflows/crowdin-download.client-ui.yml) in the main repository. For curriculum content, you will need to add a step to the [`download-curriculum.yml`](https://github.com/freeCodeCamp/i18n-curriculum/blob/main/.github/workflows/download-curriculum.yml) workflow in the [i18n-curriculum repository](https://github.com/freeCodeCamp/i18n-curriculum). The step for both workflows will be similar. For example, if you want to enable Dothraki downloads:
...
In the [shared/config/curriculum.ts](https://github.com/freeCodeCamp/freeCodeCamp/blob/main/shared/config/curriculum.ts) file, add the new language to the `notAuditedSuperBlocks` object. This lists all the superblocks which are not fully translated. Add an array of superblocks that have not been fully translated to it. For example:
...
Once you have the files, you will need to place them in the correct directory. For the curriculum challenges, you should place the block folders (individual learning modules) within the `curriculum/challenges/english/blocks/` directory for English content. For translated content, the structure is handled automatically by the i18n-curriculum submodule - translated markdown files are stored in the submodule and synchronized automatically. The client translation `.json` files will go in the `client/i18n/locales/{lang}` directory.
