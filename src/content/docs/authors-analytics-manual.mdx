...
<Steps>
  1. From the top dropdown menu, select `News`. 2. From the sidebar, click on
  `Reports`. 3. From the secondary sidebar, select `Engagement`. 4. Click on
  `Pages and Screens`. 5. In the search bar, type the subpath for the desired
  language. 6. From the dropdown under the search bar, choose `Page path and
  screen class`.
</Steps>
...
<Steps>
  1. Click on the `Add filter` button. 2. From the side navigation include
  `Author`. 3. From the `Dimensions values` dropdown, choose an author's name.
