...
Run `pnpm run create-new-project`. This opens up a command line UI that guides you through the process. Once that has finished, there should be a new challenge block that you can use to start work on the quiz. For example, if you created a project called `test-project`, it would be in `curriculum/challenges/english/blocks/test-project`.
